...
class EightQueen {
    ...
    public void recursiveSolve(int row) {
        ...
        if (row == 8) {
        	...
        	for(int c=0; c<chessBoard[0].length; c++)
        	{
        		if(isValid(row,c))
        		{
        			chessBoard[row][c] = true;
        			recursiveSolve(row+1);
        		}
        		chessBoard[row][c] = false;
        	}
    ...
    public boolean isValid(int row, int col) {
    	...
    	//goes through the same col to see if this is a valid place to put a queen
        ...
        for(int r=0;r<chessBoard.length; r++)
        {
        	for(int c=0;c<chessBoard[0].length; c++)
        	{
        		if(chessBoard[r][c] && Math.abs(row - r) == Math.abs(col - c))
        		{
        			return false;
        		}
        	}
